...
function parseDate(value) {
  ...
  const textMonth = raw.match(/^(\d{1,2})\s+([A-Za-z]{3,9})\s+(\d{2,4})$/);
  if (textMonth) {
    const day = Number(textMonth[1]);
    const month = monthMap[textMonth[2].toLowerCase()];
    let year = Number(textMonth[3]);
...
function categorize(description, amount, type) {
  ...
  const rules = [
    ...
    ["Food", /zomato|swiggy|restaurant|cafe|starbucks|food|dining|eatclub|dominos|pizza|subway|mcdonald|burger|chaayos|biryani|sweet|vada|benne|manis cafe/],
    ["Groceries", /zepto|blinkit|instamart|grocery|dmart|reliance fresh|bigbasket|more supermarket|jiomart/],
    ["Shopping", /nykaa|myntra|amazon|flipkart|shopping|ajio|meesho|h&m|zara|westside|pantaloons|reliance trends|life style|lifestyle|ikea/],
    ["Health", /pharmacy|apollo|tata 1mg|hospital|clinic|doctor|medical|diagnostic|thyrocare|manmatters|ourlittlejoys|littlejoys|mosaic wellness/],
    ...
    ["Entertainment", /netflix|prime video|hotstar|bookmyshow|spotify|movie|pvr|inox|youtube|google/],
    ["Subscriptions", /subscription|apple|google one|icloud|microsoft|notion|canva|youtubegoogle/],
    ["Investments", /zerodha|mutual fund|sip|groww|kite|nse|bse|stock|broking/],
    ["Travel", /flight|hotel|airbnb|makemytrip|goibibo|cleartrip|indigo|air india|hotel vihang/],
...
function parsePdfLines(lines, sourceName) {
  const kotakRows = parseKotakPdfLines(lines, sourceName);
  if (kotakRows.length >= 5) return kotakRows;
  return parseGenericPdfLines(lines, sourceName);
}

function parseKotakPdfLines(lines, sourceName) {
  const joined = lines.join("\n").toLowerCase();
  const looksLikeKotak = joined.includes("kotak") || joined.includes("savings account transactions") || joined.includes("withdrawal (dr.)");
  if (!looksLikeKotak) return [];

  const transactions = [];
  let previousBalance = null;
  const openingLine = lines.find((line) => /opening\s+balance/i.test(line));
  if (openingLine) {
    const openingAmounts = getMoneyMatches(openingLine);
    if (openingAmounts.length) previousBalance = openingAmounts[openingAmounts.length - 1].value;
  }

  for (const line of lines) {
    const cleanLine = String(line || "").replace(/\uFFFE/g, "-").replace(/\s+/g, " ").trim();
    if (!cleanLine || /opening\s+balance|statement generated|account statement|savings account transactions|withdrawal \(dr\.|deposit \(cr\.|end of statement|important information/i.test(cleanLine)) continue;

    const rowMatch = cleanLine.match(/^\s*(\d{1,4})\s+(\d{1,2}\s+[A-Za-z]{3,9}\s+\d{4})\s+(.+)$/);
    if (!rowMatch) continue;

    const serial = Number(rowMatch[1]);
    const dateText = rowMatch[2];
    const afterDate = rowMatch[3];
    const money = getMoneyMatches(afterDate);
    if (money.length < 2) continue;

    const balanceMatch = money[money.length - 1];
    const transactionMatch = money[money.length - 2];
    const balance = balanceMatch.value;
    let amount = transactionMatch.value;
    if (!Number.isFinite(balance) || !Number.isFinite(amount) || amount <= 0) continue;

    let type = "expense";
    if (previousBalance !== null && Number.isFinite(previousBalance)) {
      const difference = Number((balance - previousBalance).toFixed(2));
      if (Math.abs(Math.abs(difference) - amount) <= 0.05) {
        type = difference >= 0 ? "income" : "expense";
        amount = Math.abs(difference);
      } else {
        type = inferTypeFromDescription(afterDate, transactionMatch.raw);
      }
    } else {
      type = inferTypeFromDescription(afterDate, transactionMatch.raw);
    }
    ...
    const descriptionRaw = afterDate.slice(0, transactionMatch.index).trim();
    let description = descriptionRaw
      .replace(/\bUPI-\d+\b/gi, " ")
      .replace(/\bNACHDR\d+\b/gi, " ")
      .replace(/\b\d{10,}\b/g, " ")
      .replace(/\busing Payt\b/gi, "using Paytm")
      .replace(/\s+/g, " ")
      .trim();

    if (!description) description = `Kotak transaction ${serial}`;
    ...
    transactions.push({
      ...
      merchant: getMerchant(description),
      source: sourceName || "Kotak PDF statement",
      selected: true
    });
  }
...
function getMoneyMatches(text) {
  const moneyRegex = /(?<!\d)(?:₹\s*)?(?:\d{1,3}(?:,\d{2,3})+|\d+)\.\d{2}(?!\d)\s*(?:CR|DR)?/gi;
  return [...String(text || "").matchAll(moneyRegex)].map((match) => ({
    raw: match[0],
    index: match.index || 0,
    value: Math.abs(toNumber(match[0]))
  }));
}

function inferTypeFromDescription(description, amountToken) {
  const text = `${description || ""} ${amountToken || ""}`.toLowerCase();
  if (/\bcr\b|deposit|credit|salary|stipend|fellowship|refund|sent from paytm|received/.test(text)) return "income";
  return "expense";
}

function parseGenericPdfLines(lines, sourceName) {
  const transactions = [];
  ...
  lines.forEach((line) => {
    ...
    const type = inferTypeFromDescription(description, rawAmount);
